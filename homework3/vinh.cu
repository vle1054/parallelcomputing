...
__global__ void spmv (int * ptr, int * indices, float * data, float * b, float * t) {
	int i = blockIdx.x;
	int tid = threadIdx.x;
	__shared__ float partSum[32];
	partSum[tid] = 0;
	for (int j = ptr[i] + tid; j<ptr[i+1]; j+= blockDim.x) {
		partSum[tid] +=  data[j] * b[indices[j]];
	}
	__syncthreads();
	for (int z = blockDim.x/2; z > 0; z /=2) {
		if (tid < z) {
			partSum[tid] += partSum[tid+z];
		}
		__syncthreads();
	}
	t[i] = partSum[0];
...
// Declare pointers on the GPU

int * ptr_d, * indices_d;
float * data_d, * b_d, *t_d, *t_h;
...
// This will check to see that the results are the same as one another
// If the results are not the same, we will return a fail count to show
// that the program came up with a different result than the one expected
int fail = 0;
for (int k = 0; k < nr; k++) {
